...
jacobi_solution(A,b,x_exact,epsilon,max_loop,1);

sum_t = 0;
for i = 1:10
    tic;
    jacobi_solution(A,b,x_exact,epsilon,max_loop,0);
    t = toc;
    sum_t = sum_t + t;
end

fprintf('calling original Jacobi for 10 times, sum time = %6fs\n',sum_t);
...
function jacobi_solution(A,b,x_exact,epsilon,max_loop,print)
    [A_row,A_col] = size(A);
    [b_row,~] = size(b);
