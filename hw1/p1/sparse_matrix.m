% implement Jacobi iteration with sparse matrix multiplication
...
tic
jacobi_solution(A,b,x_exact,epsilon);
toc
...
function jacobi_solution(A,b,x_exact,epsilon)
    [A_row,A_col] = size(A);
    [b_row,~] = size(b);
...
while( norm(x_cur-x_next,inf) > epsilon )
        ...
        %------------------------------------
        [temp,~] = sparse_matrix_mult(R,x_cur);
        x_next = temp + g;
